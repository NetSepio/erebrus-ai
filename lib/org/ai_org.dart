...
class AiOrg {
  ...
  factory AiOrg.fromJson(Map<String, dynamic> j) => AiOrg(
    id: (j['id'] ?? j['org_id'] ?? '').toString(),
    name: (j['name'] ?? '').toString(),
    slug: j['slug']?.toString(),
    role: (j['role'] ?? 'member').toString(),
    plan: j['plan']?.toString(),
    memberCount: j['member_count'] is int ? j['member_count'] as int : null,
    privateModelCount: j['private_model_count'] is int
        ? j['private_model_count'] as int
        : null,
    pendingInviteCount: j['pending_invite_count'] is int
        ? j['pending_invite_count'] as int
        : null,
  );
