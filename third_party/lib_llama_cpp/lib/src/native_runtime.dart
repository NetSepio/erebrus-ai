...
final class NativeLlamaRuntime {
  ...
  final List<int> _pendingTokenPieceBytes = <int>[];
  ...
  Iterable<LlamaResponse> generateMessages(
    ...
    final samplingCommand = LlamaGenerateCommand(
      ...
      topP: command.topP,
      repeatPenalty: command.repeatPenalty,
  ...
  Iterable<LlamaResponse> _sampleFromEvaluatedPrompt({
    ...
    try {
      ...
      final pendingText = _flushTokenPieceBytes();
      if (pendingText.isNotEmpty) {
        final delta = stopMatcher.add(pendingText);
        if (delta.isNotEmpty) {
          yield LlamaTokenResponse(text: delta, index: generated);
        }
      }
  ...
  Map<String, Object?> _chatTemplateRequest(
    ...
    return {
      ...
      // Thinking models can otherwise spend the entire mobile token budget in
      // a visible <think> block and never reach their answer.
  ...
  Pointer<llama_sampler> _createSampler(
    ...
    final repeatPenalty = command.repeatPenalty;
    ...
    if (repeatPenalty != null && repeatPenalty != 1) {
      add(
        _bindings.llama_sampler_init_penalties(64, repeatPenalty, 0, 0),
        'repeat-penalty',
      );
    }
  ...
  String _tokenToPiece(Pointer<llama_vocab> vocab, int token) {
    ...
    try {
      ...
      // A Unicode scalar may be split across multiple llama tokens. Decoding
      // each token piece independently throws an "Unfinished UTF-8 octet
      // sequence" FormatException. Keep an incomplete trailing sequence and
      // prepend it to the next token piece.
      final bytes = <int>[
        ..._pendingTokenPieceBytes,
        ...buffer.cast<Uint8>().asTypedList(length),
      ];
      final pendingLength = _incompleteUtf8SuffixLength(bytes);
      final completeLength = bytes.length - pendingLength;
      _pendingTokenPieceBytes
        ..clear()
        ..addAll(bytes.skip(completeLength));
      return utf8.decode(
        bytes.take(completeLength).toList(growable: false),
        allowMalformed: true,
      );
    } finally {
      calloc.free(buffer);
  ...
  String _flushTokenPieceBytes() {
    if (_pendingTokenPieceBytes.isEmpty) return '';
    final bytes = List<int>.of(_pendingTokenPieceBytes);
    _pendingTokenPieceBytes.clear();
    return utf8.decode(bytes, allowMalformed: true);
  }

  static int _incompleteUtf8SuffixLength(List<int> bytes) {
    ...
    final availableLength = bytes.length - start;
